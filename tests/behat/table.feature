@editor @editor_atto @atto @atto_table_msdlt @_bug_phantomjs
Feature: Atto tables
  ...
  Scenario: Check table setting defaults on new table
    ...
    | config | value | plugin |
    | allowborders | 1 | atto_table_msdlt |
    | allowbackgroundcolour | 1 | atto_table_msdlt |
    | allowwidth | 1 | atto_table_msdlt |
  ...
  Scenario: Create a table with background colour and width with border settings off
    Given the following config values are set as admin:
    | config | value | plugin |
    | allowborders | 0 | atto_table_msdlt |
    | allowbackgroundcolour | 1 | atto_table_msdlt |
    | allowwidth | 1 | atto_table_msdlt |
    And I log in as "admin"
    And I follow "Profile" in the user menu
